...
wiimote::wiimote ()
	...
	InitializeCriticalSection(&FileLoggingLock);
...
wiimote::~wiimote ()
	...
	DeleteCriticalSection(&FileLoggingLock);
...
void wiimote::Disconnect ()
	{
	TRACE(_T("In wiimote::Disconnect"));

	if(Handle == INVALID_HANDLE_VALUE)
		return;

	_tprintf(_T("\n\n\nExtension Disconnect\tWiimote # = %d\n\n"),this->m_wiimoteNumber);
...
int wiimote::ParseInput (BYTE* buff)
	...
	switch(buff[0])
		...
		case IN_BUTTONS_ACCEL_IR:
			...
			changed |= ParseButtons(buff);
			changed |= ParseExtension(buff, 18); //Sachin : Changed  -- changed |= ParseExtension(buff, 16);
			if(!IsBalanceBoard()) {
				changed |= ParseAccel(buff);
...
//	if(true)
		...
		if(!Internal.bExtension)
			...
			_tprintf(_T("(\n\n\nExtension disconnected\tWiimote # = %d\n\n"),this->m_wiimoteNumber);
	...
	switch(Internal.ExtensionType)
		...
		case MOTION_PLUS:
			...
			bMotionPlusEnabled  = true;
				...
				FILE * fp = fopen("WiimoteMotionPlusDataDump.log","a");
				/**
				if(fp != NULL)
				{
					fprintf(fp,"\n");
					BYTE * logBuf = &buff[16];
						...
						unsigned __int64 logBufValue = *(unsigned __int64*)logBuf;
						fprintf(fp,"0x%I64x",logBufValue);

					//unsigned __int16 logBufValue = *(unsigned __int16*)logBuf;
					//fprintf(fp,"%Ix",logBufValue);
			...
			if((yaw != 0x3fff) || (pitch != 0x3fff) || (roll != 0x3fff))
				...
				// Mingyu: try to use the scale on Wiki (too fast!!)
				//float y_scale = yaw_slow  ? 0.07263f : 0.33014f; // 595 deg/s -> 8192, times 2000/440 for the fast mode
				//float p_scale = pitch_slow? 0.07263f : 0.33014f; 
				//float r_scale = roll_slow ? 0.07263f : 0.33014f;				
				...
				if(fp != NULL)
				{
					fprintf(fp,"\n%f,%f,%f",Internal.MotionPlus.Speed.Yaw  ,Internal.MotionPlus.Speed.Pitch,Internal.MotionPlus.Speed.Roll);
					fclose(fp);
				}
				...
				fp = fopen("WiimoteDataDump.log","a");
				if(fp != NULL)
				{
					fprintf(fp,"\n");
					for(int i = 0 ; i < REPORT_LENGTH ; i = i +8)
					{
						BYTE * logBuf = &buff[i];
						unsigned __int64 logBufValue = *(unsigned __int64*)logBuf;
						fprintf(fp,"0x%I64x",logBufValue);
...
//			TRACE(_T("(found extension 0x%I64x)"), type);
			_tprintf(_T("\nfound extension 0x%I64x\tWiimote # = %d\n"), type,this->m_wiimoteNumber);
			...
			static const QWORD MOTION_PLUS_EMBED   = 0x050420a40001ULL;
			static const QWORD MOTION_PLUS_EMBED2  = 0x050420A40000ULL; //Sachin
			static const QWORD MOTION_PLUS_DETECT  = 0x050020a60000ULL;					
			static const QWORD MOTION_PLUS_DETECT2 = 0x050420a60000ULL;		
			static const QWORD MOTION_PLUS_EMBED_DETECT  = 0x050020a60001ULL; // Mingyu
			static const QWORD MOTION_PLUS_EMBED_DETECT2 = 0x050420a60001ULL; // Mingyu
			static const QWORD MOTION_PLUS_EMBED_DETECT3 = 0x050420a40000ULL; // Sachin
			static const QWORD MOTION_PLUS_EMBED_DETECT4 = 0x050020A40000ULL; // Sachin
			...
			if ((type == MOTION_PLUS_DETECT) || (type == MOTION_PLUS_DETECT2) || 
					(type == MOTION_PLUS_EMBED_DETECT) || (type == MOTION_PLUS_EMBED_DETECT2) || 
					(type == MOTION_PLUS_EMBED_DETECT3) || (type == MOTION_PLUS_EMBED_DETECT4))
				{
				if(!bMotionPlusDetected) {
					TRACE(_T("Motion Plus detected!"));
					_tprintf(_T("\nMotion Plus detected!\tWiimote # = %d\n"), this->m_wiimoteNumber);
			...
			IF_TYPE(MOTION_PLUS)
				...
				Sleep(1000);												//Sachin , Sleep
				bMotionPlusDetected = true;
				bWiimoteDetectionComplete = true;
				}
				...
				_tprintf(_T("\n.. Motion Plus Embedded!\tWiimote # = %d\n"), this->m_wiimoteNumber);
				ReadAddress(REGISTER_EXTENSION_CALIBRATION, 16);
				bMotionPlusDetected = true;
				}
			else if(type == MOTION_PLUS_EMBED2) {
				if(Internal.ExtensionType == wiimote_state::MOTION_PLUS)
					break;
				Internal.ExtensionType = wiimote_state::MOTION_PLUS;
				TRACE(_T(".. Motion Plus Embedded!"));
