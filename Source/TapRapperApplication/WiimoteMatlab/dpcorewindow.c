...
void
mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[], int nPlayWindow, const mxArray *PlayWindow)
{
    int 	i,j;
    long   	pvl, pvb[16];
    int pwindow;
    
    mexPrintf("PlayWindow length %d number ", nPlayWindow);
#ifdef DEBUG
	...
	for (j = 0; j < cols; ++j) {
	    //for (i = 0; i < rows; ++i) {
        for (i = max(0, j-pwindow); i < min(rows, j+pwindow); ++i) {
            d1 = pM[i + j*rows];
            for (k = 0; k < ncosts; ++k) {
            ...
            v = INF;
            if(i==j-pwindow) pD[i + j*rows]=10000;
            if(i==j+pwindow-1) pD[i + j*rows]=10000;
	    }
	}
	free((void *)costs);
	free((void *)steps);
