...
range=tstart:tend;
log_range=['select range: ', num2str(data.time(tstart)), '  ', num2str(data.time(tend)), ' variation: ', num2str(val)];
...
if val<0.1
    ...
    log_simi=['simi to exist: ', num2str(simi), ' threshold: ', num2str(1.97)];
    if simi<1.97
        log_warning='calibration updated in applyrobustcalibration';
        disp(log_warning);
        disp(log_range);
        disp(log_simi);
        if isfield(Log, 'DoLog')
            if Log.DoLog
                fid=fopen(Log.fname, 'a');
                fprintf(fid, '%s\n%s\n%s\n', log_warning, log_range, log_simi);
                fclose(fid);
            end
        end
